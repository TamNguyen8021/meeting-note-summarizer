...
class RealSpeechService extends ChangeNotifier {
  ...
  List<String> get transcriptionSegments => List.unmodifiable(_transcriptionSegments);
    ...
    try {
      await _speechToText.listen(
        onResult: (result) {
          ...
          if (kDebugMode) {
            print('Transcription: $_lastTranscription (confidence: ${(_confidence * 100).toStringAsFixed(1)}%)');
          }
        ...
        pauseFor: const Duration(seconds: 3), // Pause when no speech for 3 seconds
  ...
  List<String> getRecentTranscriptions({int lastNSegments = 5}) {
    ...
    final start = (_transcriptionSegments.length - lastNSegments).clamp(0, _transcriptionSegments.length);
